...
func (r registerer) registerClients(_ context.Context, extra map[string]interface{}) (http.Handler, error) {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		if val, ok := endpointMap[originalPath]; ok {
			originalPath = val
		}
		...
		mergedHeaders["X-API-Path"] = []string{originalPath}
		...
		w.Header().Set("X-API-Path", originalPath)
...
func (registerer) RegisterLogger(v interface{}) {
	if l, ok := v.(Logger); ok {
		logger = l
		logger.Debug(fmt.Sprintf("[PLUGIN: %s] v1 🎫 Client-Plugin: Registered", ClientRegisterer))
	}
}
