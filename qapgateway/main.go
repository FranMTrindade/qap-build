...
func (r registerer) registerClients(_ context.Context, extra map[string]interface{}) (http.Handler, error) {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		publicEndpoint := endpointMap[req.URL.Path]
		if publicEndpoint == "" {
			publicEndpoint = req.URL.Path
		}

		// Copia e injeta os headers custom no objeto que será enviado via NATS
		mergedHeaders := map[string][]string{}
		for k, v := range resp.Header {
			mergedHeaders[k] = v
		}
		mergedHeaders["X-API-Path"] = []string{publicEndpoint}
		...
		// Headers para o cliente HTTP
		for k, hs := range resp.Header {
			for _, h := range hs {
				w.Header().Add(k, h)
			}
		}
		w.Header().Set("X-API-Path", publicEndpoint)
