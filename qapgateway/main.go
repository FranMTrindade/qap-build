package main
  ...
  func requestToken(grantType string, refreshToken string) (string, string, time.Time, error) {
    ...
    if grantType == "password" {
      form = fmt.Sprintf("grant_type=password&client_id=%s&client_secret=%s&username=%s&password=%s", "undefined", "undefined", "admin@konneqt.io", "LZNK1kguf_KI")
    } else {
      form = fmt.Sprintf("grant_type=refresh_token&client_id=%s&client_secret=%s&refresh_token=%s", "undefined", "undefined", refreshToken)
    }
