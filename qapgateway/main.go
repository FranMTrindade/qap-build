...
func (r registerer) registerClients(_ context.Context, _ map[string]interface{}) (http.Handler, error) {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		// Adiciona header X-API-Path baseado no header do KrakenD
		krakendEndpoint := req.Header.Get("X-Api-Path")
		if krakendEndpoint == "" {
			krakendEndpoint = req.URL.Path
		}
		w.Header().Set("X-API-Path", krakendEndpoint)
		w.Header().Set("X-Status", fmt.Sprintf("%d", resp.StatusCode))

		// Escreve status e body original
