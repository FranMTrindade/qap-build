package main
  ...
  func requestToken(grantType string, refreshToken string) (string, string, time.Time, error) {
    ...
    if grantType == "password" {
      form = fmt.Sprintf("grant_type=password&client_id=qap-front&client_secret=8JVH9xygHejPpB3nFH554rsm7hhpIxmj&username=admin@konneqt.io&password=LZNK1kguf_KI")
    } else {
      form = fmt.Sprintf("grant_type=refresh_token&client_id=qap-front&client_secret=8JVH9xygHejPpB3nFH554rsm7hhpIxmj&refresh_token=%s", refreshToken)
    }
